...
TIM_TypeDef *const tim_l[TIMNUM] STORED = {TIM1, TIM2, TIM3, TIM4, TIM5, NULL, NULL, NULL, TIM9, TIM10, TIM11};
    ...
    for (ch = 0; ch < 4; ch++) {
      if ( (sr & (TIM_SR_CC1IF << ch)) && TIMER_STATUS_HAS_CH(tm, ch)) {
        ...
        if (icu->started) {
          ...
          icu->count++;
          icu->last = now;
        } else if ( ((icu->port->IDR >> icu->pad) & 1) == ICU_CFG_GET_TRIGGER(icu->conf->cfg)) {
          //start!
          icu->last = now;
          icu->started = 1;
          icu->has_capture = 0;
