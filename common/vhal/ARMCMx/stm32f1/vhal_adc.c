...
ADCDriver *adcdrv[1];
vhalAdcCaptureInfo *adccapt[1];
uint16_t adcstatus[1];
VThread *adcthread[1];
ADCConversionGroup adccg[1];
...
const uint8_t _adc_smp[] STORED = {14, 20, 26, 41, 54, 68, 84, 252};
...
int vhalAdcInit(uint32_t adc, vhalAdcConf *conf) {
	...
	if (!adcstatus[adcid]) {
		...
		/* activates ADC @ 9 MHz (APB2/DIV4) ---> max is 14MHz*/
...
int vhalAdcPrepareCapture(uint32_t adc, vhalAdcCaptureInfo *info) {
	...
	for (i = 0; i < info->npins; i++) {
		...
		if (adc_ch <= 9) {
			cg->smpr2 |= (cycles) << (3 * adc_ch);
		} else {
			cg->smpr1 |= (cycles) << (3 * (adc_ch - 10));
		}
	}
	cg->cr2 = 0;//ADC_CR2_SWSTART;
